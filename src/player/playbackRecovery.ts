...
// explicit recovery: reload the stream for network faults, rebuild the media
// buffer once for media faults. If media recovery was already spent, or the
// fault is anything else, the track is dead — give up so the store can skip it.
export function decideHlsError(
  ...
  if (type === NETWORK_ERROR) return 'restart-load'
